...
module rf {
    export class BitmapData {
        ...
        static fromImageElement(img: HTMLImageElement): BitmapData {
            ...
            if(context){
                context.drawImage(<HTMLImageElement>img, 0, 0, img.width, img.height, 0, 0, img.width, img.height);
            }
            return bmd;
        }
        ...
        copyPixels(sourceBitmapData: any, sourceRect: Size, destPoint:Point2D): void {
            let{context} = this;
            if(context){
        ...
        draw(source: BitmapData /*,matrix:Matrix = null*/): void;
        draw(source: HTMLImageElement /*,matrix:Matrix = null*/): void;
        draw(source: any /*,matrix:Matrix = null*/): void //todo:draw matrix
        {
            let{context} = this;
            if(context){
    ...
    export class MaxRectsBinPack {
         ...
         * 
         */
        insert(width: number, height: number, method: number = 0): Rect {
